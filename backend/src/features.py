...
def build_features(df_clean):
    ...
    # B. High Season: April, May, July, August, October, December
    df['is_high_season'] = df['month'].isin([4, 5, 7, 8, 10, 12]).astype(int)
    
    # C. Vacances: July, August, December
    df['is_vacances'] = df['month'].isin([7, 8, 12]).astype(int)
    
    # D. Fixed national holidays count per month in Morocco
    ...
    # 6. Economic lagged variables (t-1 and t-3)
    ...
    # A. Z-Score anomalies (calculated on target first differences)
    ...
    # C. Prophet-like anomalies (Residuals from Trend + Seasonality)
...
def get_feature_list():
    """
    Returns the list of features to be used for machine learning.
    """
    return [
